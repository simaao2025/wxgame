...
/***/ (function (module, exports, __webpack_require__) {
        ...
        function inject() {
            ...
            // 添加空实现以防止 TextView 和 ImageView 错误
            if (typeof wx.insertTextView === 'undefined') {
                wx.insertTextView = function() { return Promise.resolve() }
            }
            if (typeof wx.updateTextView === 'undefined') {
                wx.updateTextView = function() { return Promise.resolve() }
            }
            if (typeof wx.removeTextView === 'undefined') {
                wx.removeTextView = function() { return Promise.resolve() }
            }
            if (typeof wx.insertImageView === 'undefined') {
                wx.insertImageView = function() { return Promise.resolve() }
            }
            if (typeof wx.updateImageView === 'undefined') {
                wx.updateImageView = function() { return Promise.resolve() }
            }
            if (typeof wx.removeImageView === 'undefined') {
                wx.removeImageView = function() { return Promise.resolve() }
            }
  ...
  /***/ (function (module, exports) {
        ...
        const windowWidth = _wx$getSystemInfoSync.windowWidth || screenWidth
        const windowHeight = _wx$getSystemInfoSync.windowHeight || screenHeight
        const _realDpr = devicePixelRatio || 1
        if (typeof GameGlobal !== 'undefined') {
            GameGlobal.__weappAdapterRealDpr = _realDpr
        }

        const innerWidth = exports.innerWidth = windowWidth * _realDpr
        const innerHeight = exports.innerHeight = windowHeight * _realDpr
        exports.devicePixelRatio = 1
        // DEBUG: log all key values so we can diagnose PC canvas sizing
        //console.log('[weapp-adapter] screenWidth='+screenWidth+' screenHeight='+screenHeight+' windowWidth='+windowWidth+' windowHeight='+windowHeight+' devicePixelRatio='+devicePixelRatio+' _realDpr='+_realDpr+' innerWidth='+innerWidth+' innerHeight='+innerHeight)
        const screen = exports.screen = {
            availWidth: screenWidth * _realDpr,
            availHeight: screenHeight * _realDpr
        }
            ...
            _createClass(HTMLElement, [{
                ...
                get: function get() {
                    if (this.style && typeof this.style.width === 'string') {
                        if (this.style.width.indexOf('%') !== -1) return window.innerWidth
                ...
                key: 'clientHeight',
                get: function get() {
                    if (this.style && typeof this.style.height === 'string') {
                        if (this.style.height.indexOf('%') !== -1) return window.innerHeight
  ...
  /***/ (function (module, exports, __webpack_require__) {
        ...
        function Canvas() {
            ...
            if (!GameGlobal.__weappAdapterMainCanvasSized) {
                GameGlobal.__weappAdapterMainCanvasSized = true
                const _wx$getSystemInfoSync = wx.getSystemInfoSync()
                const _screenWidth = _wx$getSystemInfoSync.windowWidth || _wx$getSystemInfoSync.screenWidth
                const _screenHeight = _wx$getSystemInfoSync.windowHeight || _wx$getSystemInfoSync.screenHeight
                const _devicePixelRatio = _wx$getSystemInfoSync.devicePixelRatio || 1
                canvas.width = Math.floor(_screenWidth * _devicePixelRatio)
                canvas.height = Math.floor(_screenHeight * _devicePixelRatio)
                if (canvas.style) {
                    canvas.style.width = `${_screenWidth}px`
                    canvas.style.height = `${_screenHeight}px`
                }
                ...
                canvas.getContext = function (type, attrs) {
                    ...
                    const wxDpr = info.devicePixelRatio || 1
                    const platform = info.platform || ''
                    const isPC = platform === 'windows' || platform === 'mac'
                    // On PC, godot-sdk.js forces window.devicePixelRatio>=2 AFTER
                    // weapp-adapter runs. Godot derives logical size as
                    // canvas.width / window.devicePixelRatio, so we must set
                    // canvas.width = w * window.devicePixelRatio (not wx dpr)
                    // so that Godot's logical size == w (actual window width).
                    // On other platforms wx dpr == window.dpr so either works.
                    const winDpr = window.devicePixelRatio || 1
                    const dpr = isPC ? winDpr : wxDpr
                    ...
                    //console.log('[weapp-adapter] canvas.getContext: platform='+platform+' w='+w+' h='+h+' wxDpr='+wxDpr+' winDpr='+winDpr+' isPC='+isPC+' dpr='+dpr+' canvas.width='+this.width)
                    const ctx = _getContext.call(this, type, attrs)
                    if (!GameGlobal.__weappAdapterReportedCanvasInfo) {
                        GameGlobal.__weappAdapterReportedCanvasInfo = true
                        const physicalW = this.width
                        const physicalH = this.height
                        const drawingW = ctx && typeof ctx.drawingBufferWidth === 'number' ? ctx.drawingBufferWidth : null
                        const drawingH = ctx && typeof ctx.drawingBufferHeight === 'number' ? ctx.drawingBufferHeight : null
                        const msg = drawingW != null
                            ? `[canvas] logical=${w}x${h} dpr=${winDpr} physical=${physicalW}x${physicalH} drawing=${drawingW}x${drawingH}`
                            : `[canvas] logical=${w}x${h} dpr=${winDpr} physical=${physicalW}x${physicalH}`
                        GameGlobal.__canvasInfo = msg
                        //console.log(msg)
                    }
                    return ctx
                }
  ...
  /***/ (function (module, exports, __webpack_require__) {
        ...
        const Audio = (function (_HTMLAudioElement) {
            ...
            function Audio(url) {
                ...
                // 保存回调引用，方便先 off 再 on，防止微信 InnerAudioContext 累积监听器导致内存泄漏
                const _onCanplayCb = function () {
                ...
                const _onPlayCb = function () {
                    _this.dispatchEvent({ type: 'play' })
                }
                const _onPauseCb = function () {
                    _this.dispatchEvent({ type: 'pause' })
                }
                const _onEndedCb = function () {
                    _this.dispatchEvent({ type: 'ended' })
                    _this.readyState = HAVE_ENOUGH_DATA
                }
                const _onErrorCb = function () {
                    _this.dispatchEvent({ type: 'error' })
                }
                // 先 off 再 on，确保每个事件只有一个监听器，避免微信报 memory leak 警告
                innerAudioContext.offCanplay && innerAudioContext.offCanplay(_onCanplayCb)
                innerAudioContext.onCanplay(_onCanplayCb)
                innerAudioContext.offPlay && innerAudioContext.offPlay(_onPlayCb)
                innerAudioContext.onPlay(_onPlayCb)
                innerAudioContext.offPause && innerAudioContext.offPause(_onPauseCb)
                innerAudioContext.onPause(_onPauseCb)
                innerAudioContext.offEnded && innerAudioContext.offEnded(_onEndedCb)
                innerAudioContext.onEnded(_onEndedCb)
                innerAudioContext.offError && innerAudioContext.offError(_onErrorCb)
                innerAudioContext.onError(_onErrorCb)
  ...
  /***/ (function (module, exports, __webpack_require__) {
        ...
        function touchEventHandlerFactory(type) {
            return function (event) {
                ...
                const dpr = (typeof GameGlobal !== 'undefined' && GameGlobal.__weappAdapterRealDpr) ? GameGlobal.__weappAdapterRealDpr : (wx.getSystemInfoSync().devicePixelRatio || 1)
                const scaleTouch = function (touch) {
                    const t = Object.assign({}, touch)
                    if (typeof t.clientX === 'number') t.clientX *= dpr
                    if (typeof t.clientY === 'number') t.clientY *= dpr
                    if (typeof t.pageX === 'number') t.pageX *= dpr
                    if (typeof t.pageY === 'number') t.pageY *= dpr
                    if (typeof t.screenX === 'number') t.screenX *= dpr
                    if (typeof t.screenY === 'number') t.screenY *= dpr
                    if (typeof t.x === 'number') t.x *= dpr
                    if (typeof t.y === 'number') t.y *= dpr
                    return t
                }
                touchEvent.touches = Array.prototype.map.call(event.touches || [], scaleTouch)
                touchEvent.targetTouches = Array.prototype.map.call(event.touches || [], scaleTouch)
                touchEvent.changedTouches = Array.prototype.map.call(event.changedTouches || [], scaleTouch)
